...
const CONTROL_REQUEST_TIMEOUT = 15000;
...
function Vector(config) {
    ...
    const init = function () {
        ...
        const behaviorStreamFactory = () => {
            try {
                ...
                logger.error("behavior stream error " + error);
            }
        };
    ...
    function requestControl(controlRequestPriority) {
        ...
        const promise = new Promise(function (resolve) {
            ...
            logger.debug("requested control");
            const timeoutId = setTimeout(() => {
                logger.error("Control request timeout");
                throw "Request timeout";
            }, CONTROL_REQUEST_TIMEOUT);
            behaviorStreamEmitter.once(EVENT_CONTROL_GRANTED_RESPONSE, () => {
                clearTimeout(timeoutId);
                logger.debug("control granted");
                hasControl = true;
                resolve();
            });
        });
